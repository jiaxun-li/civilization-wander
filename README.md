...
需要 Node.js 22.18 或更高版本，推荐使用当前 Node.js 24 LTS。首次运行先安装开发依赖：
...
`pnpm build` 生成 `dist/`，`pnpm preview` 用本地服务器预览与 GitHub Pages 相同的产物。`dist/` 是临时构建目录，不提交到仓库。项目使用 Vite 和 TypeScript 作为开发底座，以 React/React DOM 渲染首页、外壳和完整 Card tree；应用编排、内容模块、Card view、Reader、Map 与其消费的 V5 运行时边界已经类型化，现有 V5 数据 schema、界面行为和本地内容资源保持不变。正式页面没有远程内容、地图、字体或图片依赖。
